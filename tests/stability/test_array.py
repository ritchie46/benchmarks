...
@pytest.mark.parametrize("rechunk_method", ["p2p", "tasks"])
@pytest.mark.stability
def test_rechunk_in_memory(small_client, rechunk_method):
    x = da.random.random((50000, 50000))
    x.rechunk((50000, 20), rechunk=rechunk_method).rechunk(
        (20, 50000), rechunk=rechunk_method
    ).sum().compute()


@pytest.mark.parametrize("rechunk_method", ["p2p", "tasks"])
# @pytest.mark.skip(reason="this runs forever")
def test_rechunk_out_of_memory(small_client, rechunk_method):
